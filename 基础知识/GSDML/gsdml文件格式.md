...
- **作用**：定义子模块的实时I/O数据映射（输入/输出字节分配）。此处为空，表示该子模块**不参与实时I/O通信**，仅用于参数配置。

---

### **4. `<RecordDataList>` 参数记录配置**

xml复制

`<RecordDataList>     <ParameterRecordDataItem Index="10" Length="2" TransferSequence="0">         <!-- 参数1：置位时间 -->         <Ref DataType="Unsigned8" ByteOffset="0" DefaultValue="50"               AllowedValues="0..255" Changeable="true" Visible="true"               TextId="TID_Config_SetBIT_HoldTime" />         <!-- 参数2：通信间隔 -->         <Ref DataType="Unsigned8" ByteOffset="1" DefaultValue="200"               AllowedValues="0..255" Changeable="true" Visible="true"               TextId="TID_Config_CommInterval" />     </ParameterRecordDataItem> </RecordDataList>`

- **`Index="10"`**
    
    - 参数记录的索引号（10号记录），控制器通过此索引访问参数。
- **`Length="2"`**
    
    - 参数记录总长度为 **2字节**（对应两个Unsigned8参数）。
- **`TransferSequence="0"`**
    
    - 参数传输优先级（0表示最高优先级，优先下载到设备）。
- **参数项属性说明**（以置位时间为例）：
    
    - **`DataType="Unsigned8"`**：数据类型为无符号8位整数（0-255）。
    - **`ByteOffset="0"`**：参数在记录中的字节偏移量（第0字节）。
    - **`DefaultValue="50"`**：默认值为50（单位可能为毫秒）。
    - **`AllowedValues="0..255"`**：允许配置范围（0-255）。
    - **`Changeable="true"`**：允许运行时修改。
    - **`Visible="true"`**：在工程工具中显示此参数。
    - **`TextId="..."`**：参数名称/描述的文本引用（支持多语言）。
...
`<ModuleInfo>     <Name TextId="T_DAP1_NAME" />     <InfoText TextId="TID_ModInfo_InfoTextId_DAP8" /> </ModuleInfo>`

- **`<Name>`**：子模块名称，引用外部文本 `T_DAP1_NAME`（如“伺服参数配置”）。
- **`<InfoText>`**：子模块描述，引用 `TID_ModInfo_InfoTextId_DAP8`（如功能说明）。